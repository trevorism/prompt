...
export default {
  ...
  mounted() {
    ...
    axios.get("/api/list/question/").then(result => {
  ...
  <div>
    <div v-if="authenticated">
      <div v-for="item in questionList">
        <question :id="item.question.id" :date="item.question.createDate"
                  :user="item.question.username" :text="item.question.text" :answerMode="false"
    ...
    <div v-else>
      You must login to view all questions.
    </div>
  </div>
</template>
